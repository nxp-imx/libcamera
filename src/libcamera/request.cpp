...
{
	LOG(Request, Debug) << "==== Request::Private::complete(), cancelled_ " << cancelled_;
...
void Request::Private::doCancelRequest()
{
	...
	LOG(Request, Debug) << "==== Request::Private::doCancelRequest";
	cancelled_ = true;
	pending_.clear();
	notifiers_.clear();
	timer_.reset();
		       ...
		       std::unique_ptr<Fence> fence)
{
	...
	LOG(Request, Debug) << "==== addBuffer " << buffer << " for stream " << stream << ", fence " << fence.get() << ", buffer->_d() " << buffer->_d();
	...
	if (fence && fence->isValid())
		...
		LOG(Request, Debug) << "==== addBuffer, fence invalid";
