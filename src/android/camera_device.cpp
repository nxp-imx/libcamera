...
int CameraDevice::processCaptureRequest(camera3_capture_request_t *camera3Request)
{
  ...
  LOG(HAL, Info) << "====xxxx processCaptureRequest() fd " << hnd->data[0] << ", descriptor " << descriptor.get() << ", frameNumber_ " << descriptor->frameNumber_;
    ...
    bSuc = lastSettings_.getEntry(ANDROID_CONTROL_AE_TARGET_FPS_RANGE, &entry);
    LOG(HAL, Info) << "==== processCaptureRequest(), ANDROID_CONTROL_AE_TARGET_FPS_RANGE, bSuc " << bSuc << ", " << entry.data.i32[0] << ", " << entry.data.i32[1];
	}
